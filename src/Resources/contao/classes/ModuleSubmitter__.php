...
namespace BurkiSchererAG\BSEventSubmit;
...
/**
 * Class ModuleSubmitter
 */
class ModuleSubmitter__ extends Module
{
    ...
    protected function compile()
    {
        ...
        $this->editable = StringUtil::deserialize($this->{$this->editableFields});
        ...
        if (strpos($this->strTable, 'calendar') > 0) {
            $objEvent = new \Contao\CalendarEventsModel();
        }
        if (strpos($this->strTable, 'news') > 0) {
            $objEvent = new \Contao\NewsModel();
        }
    ...
    public function createNewEvent($objEvent)
    {
        ...
        if (strpos($this->strTable, 'news') > 0) {
            //news Archive
            $objEvent->pid     = $this->bsNewsSubmitArchive;
            $calender = $objEvent->getRelated('pid')->row();
            $objEvent->author = $calender['newsOwner'];
            $slug_seed = $objEvent->headline;
            $objEvent->date = time();
            $objEvent->time = time();
        }
